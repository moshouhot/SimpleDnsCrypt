		...
		public static bool Patch()
		{
			...
			if (version.Equals("0.6.8") || version.Equals("0.6.9") || version.Equals("0.7.0"))
			{
				...
				if (!sources.ContainsKey("relays"))
				{
					...
					{
						urls = new[] { "https://raw.githubusercontent.com/DNSCrypt/dnscrypt-resolvers/master/v3/relays.md", "https://download.dnscrypt.info/resolvers-list/v3/relays.md", "https://gitee.com/ilangel/dnscrypt-resolvers/raw/master/v3/relays.md" },
			...
			{
				//added: Gitee backup sources for Chinese users
				var sources = DnscryptProxyConfigurationManager.DnscryptProxyConfiguration.sources;

				// Update public-resolvers sources
				if (sources.ContainsKey("public-resolvers"))
				{
					var publicResolvers = sources["public-resolvers"];
					var currentUrls = publicResolvers.urls.ToList();
					var giteeUrl = "https://gitee.com/ilangel/dnscrypt-resolvers/raw/master/v3/public-resolvers.md";
					if (!currentUrls.Contains(giteeUrl))
					{
						currentUrls.Add(giteeUrl);
						publicResolvers.urls = currentUrls.ToArray();
					}
				}

				// Update relays sources
				if (sources.ContainsKey("relays"))
				{
					var relays = sources["relays"];
					var currentUrls = relays.urls.ToList();
					var giteeUrl = "https://gitee.com/ilangel/dnscrypt-resolvers/raw/master/v3/relays.md";
					if (!currentUrls.Contains(giteeUrl))
					{
						currentUrls.Add(giteeUrl);
						relays.urls = currentUrls.ToArray();
					}
				}
