		...
		private async Task<string> StartRemoteUpdateDownload(RemoteUpdate remoteUpdate)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(signature) && installer != null)
				{
					...
					var loadedSignature = Core.LoadSignatureFromString(s[1], trimmedComment, s[3]);
					var publicKey = Core.LoadPublicKeyFromString(Global.ApplicationUpdatePublicKey);
					var valid = Core.ValidateSignature(installer, loadedSignature, publicKey);
